...
export default function FlaglePage() {
  const { canPlay, markPlayed, hoursUntilReset, ready } = useDaily('flagle');
  ...
  if (!canPlay) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-6" style={{ background: "linear-gradient(135deg, #F0EBFF 0%, #E8F4FF 50%, #F0FFF8 100%)" }}>
        ...
        <Link href="/games" className="font-bold text-sm no-underline" style={{ color: "#F59E0B" }}>Back to games</Link>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="min-h-screen flex flex-col items-center pb-8" style={{ background: 'linear-gradient(135deg, #F0EBFF 0%, #E8F4FF 50%, #F0FFF8 100%)', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      <div className="sticky top-0 w-full z-20 flex items-center justify-between px-4 h-14"
        ...
        <span className="font-black text-base" style={{ color: '#1e1b4b' }}>Flagle</span>
      ...
      <div className="w-full max-w-md px-4 mt-6 flex flex-col items-center flex-1">
        {/* Game canvas area */}
        <div className="relative rounded-2xl overflow-hidden mb-8" style={{ width: '100%', maxWidth: 320, aspectRatio: '3/2', boxShadow: "0 16px 40px rgba(245,158,11,0.15)", border: "2px solid rgba(255,255,255,0.9)", background: 'rgba(255,255,255,0.5)', backdropFilter: 'blur(8px)' }}>
          <img
              ...
              >
                {!isRevealed && (
                    <motion.div 
                        ...
                        style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg, transparent 0%, rgba(245,158,11,0.1) 50%, transparent 100%)' }}
                    />
                )}
              </motion.div>
            );
